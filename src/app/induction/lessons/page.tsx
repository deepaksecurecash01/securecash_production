...
export default function LessonsPage() {
  ...
  const handleLessonPass = (): void => {
    // Compute isLastLesson BEFORE calling completeLesson.
    // completeLesson calls setState which is async — reading progress after
    // calling it would read stale values and break the navigation decision.
    const isLastLesson = progress.completedLessonIds.length + 1 >= totalLessons;
  ...
  return (
    <InductionGuard>
      {/* Hero — rendered directly in the page, no wrapping layout needed */}
      <InductionHero />
            ...
            {!showQuiz ? (
              ...
              >
                {currentLesson.modules.map((block, index) => (
                  <LessonModuleBlock key={index} block={block} />
                ))}
              </div>
            ) : (
        ...
        {!showQuiz && (
          <div className="relative w-full h-[250px] mt-12">
            <div
              ...
              style={{
                backgroundImage:
                  "url('/images/induction/induction-banner.jpg')",
              }}
            />
